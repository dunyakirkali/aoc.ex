...
input
  |> String.trim
  |> String.split("\n", trim: true)
  |> Enum.reduce(matrix, fn(line, grid) ->
    splits = String.split(line, " through ")
    ...
    sx = List.first(from_x_and_y)
    ex = List.first(to_x_and_y)
    sy = List.last(from_x_and_y)
    ey = List.last(to_x_and_y)
    
    new_grid = 
      case action do
      "turn on" ->
        for x <- sx..ex, y <- sy..ey, into: grid, do: { {x,y}, Map.get(grid, {x,y}) + 1 }
      "turn off" ->
        for x <- sx..ex, y <- sy..ey, into: grid do
          if Map.get(grid, {x,y}) == 0 do
            {{x,y},  0}
          else
      ...
      "toggle" ->
        for x <- sx..ex, y <- sy..ey, into: grid, do: { {x,y}, Map.get(grid, {x,y}) + 2 }
      end
    new_grid
  end)
  |> Map.values
