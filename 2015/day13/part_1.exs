...
scores = lines
  |> Enum.reduce([], fn(line, acc) ->
    [name, _, inc_dec, score, _, _, _, _, _, _, with_name] = line
    if inc_dec == "lose" do
      acc ++ [%{{name, String.slice(with_name, 0, String.length(with_name) - 1)} => -1 * String.to_integer(score)}]
    else
      acc ++ [%{{name, String.slice(with_name, 0, String.length(with_name) - 1)} => String.to_integer(score)}]
    end
  end)
  |> IO.inspect
    
Solver.permutations(people)
  |> Enum.map(fn(setup) ->
    setup ++ [List.first(setup)]
  end)
  |> Enum.map(fn(setup) ->
    Enum.chunk_every(setup, 2, 1, :discard)
      |> Enum.map(fn(ps) -> 
        po = List.first(ps)
        pt = List.last(ps)
        scores
          |> Enum.filter(fn(score) ->
            peers = Map.keys(score)
              |> List.first
              |> Tuple.to_list
            Enum.member?(peers, po) && Enum.member?(peers, pt)
          end)
          |> Enum.map(fn(res) ->
            Map.values(res)
          end)
