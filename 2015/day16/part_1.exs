...
input
  ...
  |> Enum.map(fn(aunt) ->
    ...
    rv1 = findings[var1 |> String.slice(0, String.length(var1) - 1) |> String.to_atom]
    rv2 = findings[var2 |> String.slice(0, String.length(var2) - 1) |> String.to_atom]
    rv3 = findings[var3 |> String.slice(0, String.length(var3) - 1) |> String.to_atom]

    diff = abs(rv1 - vv1) + abs(rv2 - vv2) + abs(rv3 - vv3)
    {number |> String.slice(0, String.length(number) - 1) |> String.to_integer, diff}
  end)
  |> Enum.min_by(fn(list) ->
    elem(list, 1)
  end)
